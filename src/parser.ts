...
import { newNowExpression } from "./ast.ts";
...
export class Parser {
  ...
  private parseSnapExpression(): SnapExpression {
    ...
    if (curToken.type !== TokenType.MODIFIER) {
      this.addError(
        `Expected snap modifier literal, got "${curToken.literal}"`,
      );
  ...
  private parseExpression(): Expression | undefined {
    ...
    if (TokenType.NOW === curToken.type) {
      ...
      TokenType.PLUS === curToken.type || TokenType.MINUS === curToken.type
    ) {
      return this.parseModifierExpression();
    } else if (
      TokenType.SLASH === curToken.type || TokenType.AT === curToken.type
    ) {
